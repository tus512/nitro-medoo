...
func (s *ExecutionEngine) Reorg(lastMsgIdxToKeep arbutil.MessageIndex, newMessages []arbostypes.MessageWithMetadataAndBlockHash, oldMessages []*arbostypes.MessageWithMetadata) ([]*execution.MessageResult, error) {
...
func (s *ExecutionEngine) sequenceTransactionsWithBlockMutex(header *arbostypes.L1IncomingMessageHeader, txes types.Transactions, hooks *arbos.SequencingHooks) (*types.Block, error) {
	...
	err = s.consensus.WriteMessageFromSequencer(msgIdx, msgWithMeta, *msgResult)
	if err != nil {
		return nil, err
	}
...
func (s *ExecutionEngine) sequenceDelayedMessageWithBlockMutex(message *arbostypes.L1IncomingMessage, delayedMsgIdx uint64) (*types.Block, error) {
	...
	err = s.consensus.WriteMessageFromSequencer(msgIdx, messageWithMeta, *msgResult)
	if err != nil {
		return nil, err
	}
