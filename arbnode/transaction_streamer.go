...
func (s *TransactionStreamer) addMessagesAndReorg(batch ethdb.Batch, lastMsgIdxToKeep arbutil.MessageIndex, newMessages []arbostypes.MessageWithMetadataAndBlockHash) error {
	...
	messagesResults, err := s.exec.Reorg(lastMsgIdxToKeep + 1, newMessages, oldMessages)
	if err != nil {
		return err
	}
