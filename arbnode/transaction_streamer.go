...
func (s *TransactionStreamer) reorg(batch ethdb.Batch, newHeadMsgIdx arbutil.MessageIndex, newMessages []arbostypes.MessageWithMetadataAndBlockHash) error {
	...
	messagesResults, err := s.exec.Reorg(newHeadMsgIdx, newMessages, oldMessages)
	if err != nil {
		return err
	}
